...
def load_grayscale_images(dataset_path: str, batch_size: int, images_height: int, images_width: int, epochs: int = 1):

    # carrega
    paths_dataset = tf.data.Dataset(f"{dataset_path}/**/*", shuffle=True)
    ...
    return images_dataset.repeat(epochs).prefetch(tf.data.AUTOTUNE)


def _extract_label(path: tf.Tensor, number_of_classes) -> (tf.Tensor, tf.Tensor):
...
def _make_image_loader(images_height: int, images_width: int) -> Callable:
    ...
    def loader(path: tf.Tensor, label: tf.Tensor) -> (tf.Tensor, tf.Tensor):
        ...
        image = tf.io.decode_raw(raw, channels=1, dtype=tf.float32, expand_animations=False)
